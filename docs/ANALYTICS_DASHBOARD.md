...
## Автообновление дашборда-артефакта «Проявка» (облачный агент)

Дашборд https://claude.ai/code/artifact/8340045d-5ef9-4822-941d-0b33a55bb501
не тянет данные сам (артефакты не делают произвольные сетевые запросы) —
его БД (`db`-возможность) обновляется только через Клода. Раз в сутки
это делает облачный агент (скилл `schedule`), который читает готовые
агрегаты из этой же Google-таблицы (см. выше) — не с сервера напрямую,
у облачного агента к серверу нет доступа.

Чтобы агент мог прочитать таблицу без сервис-аккаунта и логина, нужно
**один раз, вручную, через веб-интерфейс Google** опубликовать в
интернет **только 5 вкладок-агрегатов** (не «Пользователи» — там
username/Telegram ID):

1. Открыть таблицу → **Файл → Опубликовать в интернете**.
2. В выпадающем списке (по умолчанию «Весь документ») выбрать **по
   очереди каждую из 5 вкладок**: «Воронка», «Источники», «Продажи»,
   «Активность по дням», «Вовлечённость» — и для каждой отдельно
   нажать «Опубликовать», формат — **значения, разделённые запятыми
   (.csv)**.
3. Каждая публикация даёт свою ссылку вида
   `https://docs.google.com/spreadsheets/d/e/<токен>/pub?gid=<gid>&single=true&output=csv`
   — **скопировать все 5 ссылок** и передать их Клоду (или сохранить в
   `.env`/заметке) для настройки агента.

Важно: публикуется именно вкладка, а не вся таблица — «Пользователи»
остаётся закрытой, если её отдельно не публиковать.
