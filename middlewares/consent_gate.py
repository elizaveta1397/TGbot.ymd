...
# не из чего выбирать. «Читать политику» на экране согласия — обычная
# url-кнопка (keyboards/consent.py), Telegram открывает её на стороне
# клиента и не шлёт боту callback_query вовсе, поэтому в этом списке
# не участвует.
ALLOWED_COMMANDS = {"/start", "/policy"}
ALLOWED_CALLBACK_DATA = {"consent_accept"}
...
class ConsentGateMiddleware(BaseMiddleware):
    ...
    в users — то есть который ещё не нажал «Даю согласие» на экране
